...
def keyword_spot(spec):
    """ Keyword detection for data preprocess           用于数据预处理的关键词检测
        For VTCK data I truncate last 80 frames of trimmed audio - "Call Stella"
    :return: 80 frames spectrogram   返回后80帧的数据
    """
    return spec[:, -config.tdsv_frame:]  # 返回后config.tdsv_frame列的数据


def random_batch(speaker_num=config.N, utter_num=config.M, shuffle=True, noise_filenum=None, utter_start=0):
    """ Generate 1 batch.
        For TD-SV, noise is added to each utterance.                对于TDSV，给每一个话语添加噪声
        For TI-SV, random frame length is applied to each batch of utterances (140-180 frames)
                                                                    对于TISV给每个批次的话语随机选择帧数140-180
        speaker_num : number of speaker of each batch               每个批次说话人的个数（4）
        utter_num : number of utterance per speaker of each batch   每个批次每个说话人的话语数（5）
        shuffle : random sampling or not                            选择是否随机样本
        noise_filenum : specify noise file or not (TD-SV)           是否指定噪音
        utter_start : start point of slicing (TI-SV)                选择切片的起始点
    :return: 1 random numpy batch (frames x batch(NM) x n_mels)     返回一个随机的批次数组（frames * NM * n_mels)
    """

    # 加载数据的路径，选择训练的路径或者测试的路径
    # config.train_path train_tisv或者train_tdsv
    # config.test_path  test_tisv或者test_tdsv
    if config.train:
    ...
    if config.tdsv:
        ...
        path = os.path.join(path, np_file)  # numpy文件的路径，对于TDSV只有一个numpy文件
        utters = np.load(path)              # 加载处理好的numpy文件   utters.shape = [762,40,180]
        if shuffle:
            np.random.shuffle(utters)       # shuffle for random sampling
        utters = utters[:speaker_num]       # 选择 N speakers,得到 utters.shape = [4,40,180]

        # concat utterances (M utters per each speaker)
        # ex) M=2, N=2 => utter_batch = [speaker1, speaker1, speaker2, speaker2]

        # utter_batch.shape = [40,3600] 4*5*180=3600
        ...
        if noise_filenum is None:
            noise_filenum = np.random.randint(0, config.noise_filenum)          # 随机返回0-16的一个整数，然后加载该噪声文件
        noise = np.load(os.path.join(config.noise_path, "noise_%d.npy"%noise_filenum))  # 加载噪音的npy文件

        utter_batch += noise[:,:utter_batch.shape[1]]   # add noise to utterance

        utter_batch = np.abs(utter_batch) ** 2  # 计算utter-batch各元素的绝对值的平方
    ...
    # TI-SV
        ...
        if shuffle:
            selected_files = random.sample(np_file_list, speaker_num)  # 在np_file_list中随机选择 speaker_num个说话人
        else:
            selected_files = np_file_list[:speaker_num]                # 如果不是随机的话，直接读取前四个说话人的数据

        utter_batch = []
        for file in selected_files:  # 依次加载 speaker_num 个说话人的数据
            utters = np.load(os.path.join(path, file), allow_pickle=True)        # load utterance spectrogram of selected speaker
            if shuffle:
                utter_index = np.random.randint(0, utters.shape[0], utter_num)   # 在每个说话人中选择 utter_num 个话语
        ...
        utter_batch = np.concatenate(utter_batch, axis=0)     # utterance batch [20, 40, 80]  4*5=20;40维；80帧

        if config.train:
            frame_slice = np.random.randint(140,181)          # 对于训练阶段，随机选择输入的切片大小 140-181
            frame_slice = np.array(frame_slice)         # 使用mfcc特征 增加的操作
            utter_batch = utter_batch[:,:,:frame_slice]
        else:
            utter_batch = utter_batch[:,:,:160]               # 对于测试阶段，统一选择输入的批次大小为160帧

    utter_batch = np.transpose(utter_batch, axes=(2,0,1))     # 转换维度，从[20,40,180]->[180,20,40]
...
def similarity(embedded, w, b, N=config.N, M=config.M, P=config.proj, center=None):  # 定义相似度矩阵
    ...
    """
    embedded_split = tf.reshape(embedded, shape=[N, M, P])  # 将输入的embedded转换成[N,M,P]的形状  [4,5,64]
