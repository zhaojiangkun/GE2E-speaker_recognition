...
audio_path = r'/home/ygd/Python_work/ge2e/data/vctk/wav48_silence_trimmed'
...
def extract_noise():
    ...
    os.makedirs(config.noise_path, exist_ok=True)              # 新建一个文件目录保存在config.noise_path
    total = len(os.listdir(clean_path))                        # clean_path路径下所有文件的长度
    batch_frames = config.N * config.M * config.tdsv_frame     # TD-SV frame number of each batch  4*5*80
    stacked_noise = []  # 创建一个堆叠噪声列表
    stacked_len = 0     # 堆叠的长度
    k = 0
...
def save_spectrogram_tdsv():
    ...
    for folder in os.listdir(audio_path):
        ...
        # aa = os.path.splitext(os.path.basename(utter_path))[0][-8:-5]
        # print(aa)
        if os.path.splitext(os.path.basename(utter_path))[0][-8:-5] != '001':  # if the text utterance doesn't exist pass
...
def save_spectrogram_tisv():
    ...
    utter_min_len = (config.tisv_frame * config.hop + config.window) * config.sr    # lower bound of utterance length 14600
    ...
    for i, folder in enumerate(os.listdir(audio_path)):
        ...
        for utter_name in os.listdir(speaker_path):
            ...
            intervals = librosa.effects.split(utter, top_db=20)         # voice activity detection 噪声活动检测
...
def tisv():
    ...
    for i, folder in enumerate(os.listdir(audio_path)):
        ...
        for utter_name in os.listdir(speaker_path):
            ...
            for interval in intervals:
                if (interval[1]-interval[0]) > utter_min_len:           # If partial utterance is sufficient long,
                    ...
                    S = librosa.feature.mfcc(utter_part,sr=sr,n_mfcc=40)
        ...
        utterances_spec = np.array(utterances_spec[0])
        print(utterances_spec.shape)
        if i<train_speaker_num:      # save spectrogram as numpy file
            np.save(os.path.join(config.train_path, "speaker%d.npy"%i), utterances_spec)
        else:
