...
def train(path):
    ...
    with tf.variable_scope("lstm"):
        ...
        # outputs = outputs[-1]
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_bw=lstm, cell_fw=lstm, inputs=batch, dtype=tf.float32)
        out1, out2 = outputs

        out = (out1 + out2) / 2
        print(out.shape)
        outputs = out[-1]
        embedded = normalize(outputs)
    ...
    print("embedded size: ", embedded.shape)  # （20,64）

    # loss
    sim_matrix = similarity(embedded, w, b)
    print("similarity matrix size: ", sim_matrix.shape)  # (20,4)
...
def test(path):
    ...
    with tf.variable_scope("lstm"):
        ...
        print(out1.shape)
        print(out2.shape)
        out = (normalize(out1) + normalize(out2)) / 2
        print(out.shape)
        out = out[-1]
        embedded = normalize(out)
    ...
    with tf.Session() as sess:
        ...
        print("\nEER : %0.4f (thres:%0.2f, FAR:%0.2f, FRR:%0.2f)"%(EER,EER_thres,EER_FAR,EER_FRR))
